#!/usr/bin/env python3
"""Restore captured iPhone wall texture and recess coarse support behind its skin.

No colors, texture samples, copied material or points are created. Fine-grain
centers and shapes stay exact; only coarse captured support moves normally. Undo the old lower-cabinet reference-material component while
preserving every other accepted component. Use local fine iPhone points to
locate the observed skin; recess coarse captured layers behind the local fine-grain skin and constrain
their normal covariance, retaining original opacity and tangent footprint. Actual grain/lighting stays.
"""
...
def main():
 ...
 anchor=(w>0)&(abs(dg)<.025)&(maxs<.0035)&(aa>.35)&(np.ptp(rgb,axis=1)<.2);ai=np.flatnonzero(anchor);tree=cKDTree(p[ai,:2]);candidate=(w>0)&(abs(dg)<R['clear_global_depth_band'])&(maxs>R['source_coarse_maxsigma_min'])&(aa>.01);ci=np.flatnonzero(candidate);dist,ni=tree.query(p[ci,:2],k=R['local_skin_neighbors']);neighbor_d=dg[ai[ni]];local_offset=np.median(neighbor_d,axis=1);mad=1.4826*np.median(abs(neighbor_d-local_offset[:,None]),axis=1);good=(dist[:,-1]<R['local_skin_max_neighbor_distance'])&(mad<.008);dl=dg[ci]-local_offset;tol=np.maximum(R['local_offset_tolerance_min'],R['local_offset_MAD_factor']*mad);bad=good&(abs(dl)>tol);removed=np.empty(0,dtype=np.int64)
 ...
 report={'status':'Unreviewed captured-texture-only wall cleanup','baseline':str(a.baseline),'baseline_hashes':{'iphone':sha256_file(a.baseline/'iphone.ply'),'reference':sha256_file(a.baseline/'reference-patches.ply')},'original_phone':prior['iphone'],'original_phone_sha256':prior['iphone_sha256'],'omitted_legacy_material_component':omitted,'recipe':R,'counts':{'original_phone_rows_restored':len(restored),'reference_rows_legacy_weight_reduced':len(drop),'fine_original_skin_anchors':len(ai),'coarse_candidates':len(ci),'coarse_candidates_rejected_uncertain':int((~good).sum()),'coarse_offsurface_rows_attenuated':len(removed),'coarse_normal_covariances_constrained':len(ei),'coarse_support_centers_recessed':len(ei),'maximum_coarse_center_displacement':float(np.max(np.linalg.norm(newp-p[ei],axis=1)))},'changes':summary,'added_rows':0,'source_field_policy':'Captured DC, fine-grain positions/covariance and original restored opacity remain exact. Coarse captured support is recessed behind local fine skin along the measured normal; tangent covariance retained, normal covariance constrained. No new/copy/densified texture, no synthetic backing, no lighting normalization.','checks':checks,'generator_sha256':sha256_file(a.out/'generator.py'),'review_required':['wall-material-close','wall-material-grazing','left-wall','mattress-top'],'limitations':['Natural captured tone variation remains; this does not aim for a perfectly uniform synthetic finish.','Coarse-source normal relocation must be reviewed for boundary continuity and natural local shading; original fine grain is unchanged.'],'remote_publish':False};(a.out/'report.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps({'counts':report['counts'],'changes':summary,'checks':checks},indent=2),flush=True)
