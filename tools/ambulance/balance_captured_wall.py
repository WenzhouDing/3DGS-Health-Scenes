...
R.update({'photometry_grid_step':.008,'photometry_local_bandwidth':.035,'fine_max_luma_bias':.14,'fine_min_contrast_scale':.70,'coarse_residual_contrast':.35,'local_skin_max_neighbor_distance':.045,'recess_quantile':.005,'recess_margin':.018,'coarse_tangent_sigma_limit':.04,'fine_optical_depth_target':2.,'fine_density_neighbors':16,'fine_tangent_sigma_limit':.00165,'fine_tangent_major_sigma_limit':.0018,'fine_max_footprint_scale':2.5,'fine_normal_sigma_limit':.00035,'fine_aspect_limit':2.})
...
def main():
 ...
 target_e=np.minimum(np.stack([target_g**2/aspect,target_g**2*aspect],axis=1),.0018**2)
 ...
 bias=np.clip(ftarget-flocal,-.07,R['fine_max_luma_bias']);contrast=np.clip(.16/np.maximum(fstd,1e-6),R['fine_min_contrast_scale'],1.)
 ...
 desired=ctarget+R['coarse_residual_contrast']*(luma[neutral]-clocal);cluma=luma[neutral]+w[neutral]*np.clip(desired-luma[neutral],-.22,.28)
